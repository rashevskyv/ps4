...
header:
  ...
  overlay_filter: 0.5
  caption:
excerpt: 'Полное руководство по взлому PS4 / PS4 PRO на стоковой прошивке версии 5.05 (5.07) и ниже<br />**Последнее изменение:** 21 октября'
---
...
HEN и MiraCFW можно запросто установить на любую приставку с прошивкой {% include /vars/sys_version.txt %} или ниже. Однако Mira использовать в данный момент для конечного пользователя строго не рекомендуется! Запомните это и используйте только HEN.
...
HEN же нужен исключительно для одной цели - давать пользователям возможность запускать не подписанный код (проще говоря - пиратские дампы игр). Mira сама по себе этого не умеет!
...
+ Запускать бекапы игр (дампы) от PS4 и PS2
